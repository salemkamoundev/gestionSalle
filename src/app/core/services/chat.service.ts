...
export class ChatService {
  ...
  getMessages(clientUid: string): Observable<ChatMessage[]> {
    ...
    return collectionData(q, { idField: 'id' }).pipe(
      map((msgs: any[]) => {
        ...
        ).map(m => ({ ...m, isAdmin: m.senderId === 'ADMIN' })) as ChatMessage[];
      })
    );
  }
  ...
  async sendMessage(text: string, senderUid: string, receiverUid: string, senderEmail: string = '') {
    ...
    const convUpdate: any = { uid: clientUid, lastMessage: text, lastMessageTime: serverTimestamp() };
  ...
  // --- COMPTEUR POUR CLIENT (Requis pour la pastille) ---
  getUnreadCountForClient(clientUid: string): Observable<number> {
    // Compte messages envoyés par ADMIN à ce client, non lus
    const q = query(
      collection(this.firestore, 'messages'),
    ...
    return new Observable(obs => onSnapshot(q, s => obs.next(s.size)));
  }
}
