    ...
    <div class="flex h-screen bg-slate-50 overflow-hidden relative">
      ...
      <aside class="fixed inset-y-0 left-0 z-50 w-72 bg-slate-900 text-white flex flex-col shadow-2xl transition-transform duration-300 ease-in-out md:relative md:translate-x-0"
        ...
        <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto custom-scrollbar">
          ...
          <a routerLink="/admin/payments" routerLinkActive="bg-purple-600 text-white shadow-lg" class="flex items-center px-4 py-3 rounded-lg text-slate-300 hover:bg-slate-800 hover:text-white transition cursor-pointer">
        ...
        <div class="p-4 border-t border-slate-800 space-y-3">
          <button (click)="mockService.resetAndSeed()" class="w-full flex items-center justify-center px-4 py-2 bg-orange-500/10 hover:bg-orange-600 border border-orange-500/50 text-orange-400 hover:text-white rounded-lg transition cursor-pointer text-xs font-bold uppercase tracking-wide"><span class="material-icons text-sm mr-2">science</span> Générer Données</button>
