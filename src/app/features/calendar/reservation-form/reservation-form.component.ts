...
import { TeamService } from '../../../core/services/team.service'; // <--- AJOUT
...
@Component({
  ...
  template: `
    <div class="max-w-5xl mx-auto bg-white p-8 rounded-xl shadow-lg mt-6 border border-slate-100 relative" (click)="closeDropdown()">
      ...
      <form [formGroup]="form" (ngSubmit)="onSubmit()" class="space-y-8">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div class="space-y-6">
            ...
            <div class="space-y-4">
               <div class="flex justify-between items-end border-b border-slate-100 pb-2">
                 <div class="flex items-center gap-2">
                   <h3 class="text-sm font-bold text-slate-400 uppercase tracking-wider">Personnel / Staff</h3>
                ...
                @for (staff of servers(); track staff.id) { 
                  <div class="relative group rounded-lg border transition-all duration-200 select-none bg-white hover:shadow-md" [class.border-emerald-500]="isServerSelected(staff.id!)" [class.bg-emerald-50]="isServerSelected(staff.id!)">
                    <div (click)="toggleServer(staff.id!)" class="p-2 flex items-center space-x-3 cursor-pointer">
                      <div class="w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs" [class.bg-emerald-500]="isServerSelected(staff.id!)" [class.text-white]="isServerSelected(staff.id!)" [class.bg-slate-200]="!isServerSelected(staff.id!)" [class.text-slate-500]="!isServerSelected(staff.id!)">
            ...
            <div class="space-y-4">
               <div class="flex justify-between items-end border-b border-slate-100 pb-2">
                 <h3 class="text-sm font-bold text-slate-400 uppercase tracking-wider">Prestataires / Équipes</h3>
                 <span class="text-xs font-bold px-2 py-1 rounded bg-slate-100 text-slate-600">{{ getSelectedTeamCount() }} sel.</span>
               </div>
              <div class="grid grid-cols-1 gap-2 max-h-60 overflow-y-auto pr-1">
                @for (team of teams(); track team.id) { 
                  <div (click)="toggleTeam(team.id!)" 
                       class="flex items-center justify-between p-3 rounded-lg border cursor-pointer transition-all hover:shadow-sm"
                       [class.border-purple-500]="isTeamSelected(team.id!)" 
                       [class.bg-purple-50]="isTeamSelected(team.id!)"
                       [class.border-slate-200]="!isTeamSelected(team.id!)">
                    
                    <div class="flex items-center gap-3">
                      <div class="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold"
                           [class.bg-purple-500]="isTeamSelected(team.id!)" [class.text-white]="isTeamSelected(team.id!)"
                           [class.bg-slate-200]="!isTeamSelected(team.id!)" [class.text-slate-600]="!isTeamSelected(team.id!)">
                        <span class="material-icons text-sm">{{ getTeamIcon(team.type) }}</span>
                      </div>
                      <div>
                        <p class="text-sm font-bold text-slate-700">{{ team.nom }}</p>
                        <p class="text-[10px] text-slate-500 uppercase">{{ team.type }}</p>
                      </div>
                    </div>
                ...
                @if(teams().length === 0) {
                  <p class="text-xs text-slate-400 italic p-2">Aucune équipe configurée.</p>
                }
              </div>
            </div>
        ...
        <div class="flex justify-end space-x-3 pt-6 border-t border-slate-100">
          ...
          <button type="button" (click)="onPrintContract()" class="mr-auto px-5 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-lg font-bold shadow-md flex items-center gap-2">
    ...
    @if (showClientModal()) { 
      <div class="fixed inset-0 z-[60] bg-black/50 flex items-center justify-center">
        <div class="bg-white p-6 rounded-lg shadow-xl">
          <h3 class="font-bold mb-4">Nouveau Client</h3>
          <form [formGroup]="quickClientForm" (ngSubmit)="saveQuickClient()">
            <div class="grid grid-cols-2 gap-4">
              <div><label class="block text-xs font-bold text-slate-500 mb-1">Nom *</label><input formControlName="nom" class="w-full px-3 py-2 border rounded-lg uppercase"></div>
              <div><label class="block text-xs font-bold text-slate-500 mb-1">Prénom *</label><input formControlName="prenom" class="w-full px-3 py-2 border rounded-lg capitalize"></div>
            </div>
            <div class="grid grid-cols-2 gap-4">
              <div><label class="block text-xs font-bold text-slate-500 mb-1">CIN</label><input formControlName="cin" class="w-full px-3 py-2 border rounded-lg"></div>
              <div><label class="block text-xs font-bold text-slate-500 mb-1">Date CIN</label><input formControlName="dateCin" type="date" class="w-full px-3 py-2 border rounded-lg"></div>
            </div>
            <div><label class="block text-xs font-bold text-slate-500 mb-1">Téléphone *</label><input formControlName="telephone" type="tel" class="w-full px-3 py-2 border rounded-lg"></div>
            <button type="submit" [disabled]="quickClientForm.invalid" class="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3 rounded-lg mt-4 shadow">Enregistrer Client</button>
          </form>
        </div>
      </div> 
    }
    ...
    @if (showStaffModal()) { 
      <div class="fixed inset-0 z-[60] bg-black/50 flex items-center justify-center">
        <div class="bg-white p-6 rounded-lg shadow-xl">
          <h3 class="font-bold mb-4">Nouveau Staff</h3>
          <form [formGroup]="quickStaffForm" (ngSubmit)="saveQuickStaff()">
            <input formControlName="nom" placeholder="Nom" class="block w-full border p-2 mb-2 rounded">
            <input formControlName="email" placeholder="Email" class="block w-full border p-2 mb-2 rounded">
            <input formControlName="telephone" placeholder="Tél" class="block w-full border p-2 mb-2 rounded">
            <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded">Sauver</button>
            <button type="button" (click)="closeStaffModal()" class="ml-2 text-red-500">Fermer</button>
          </form>
        </div>
      </div> 
    }
...
export class ReservationFormComponent implements OnInit {
  ...
  private teamService = inject(TeamService); // <--- SERVICE INJECTÉ
  ...
  teams = toSignal(this.teamService.getAll(), { initialValue: [] }); // <--- CHARGEMENT DES ÉQUIPES
  ...
  filteredClients = computed(() => { 
    ...
    return term ? all.filter(c => c.nom.toLowerCase().includes(term) || c.prenom.toLowerCase().includes(term) || c.telephone.includes(term)) : all; 
  });
  ...
  form = this.fb.group({ 
    ...
    clientName: [''], 
    assignedServerIds: [[] as string[]], // Staff
    assignedTeamIds: [[] as string[]],   // <--- ÉQUIPES AJOUTÉES
    status: ['CONFIRMED'], 
    totalPrice: [0], 
    advance: [0] 
  });

  quickClientForm = this.fb.group({ nom: ['', Validators.required], prenom: ['', Validators.required], cin: [''], dateCin: [''], telephone: ['', Validators.required], createdAt: [new Date().toISOString()] });
  ...
  ngOnInit() {
    ...
    if (id) {
      ...
      if (timeParam) setTimeout(() => this.trySelectSlot(timeParam), 200);
    } else {
  ...
  onPrintContract() {
    if (this.form.value) this.pdfService.generateContract(this.form.value);
  }
  ...
  loadReservation(id: string) { 
    this.reservationService.getById(id).subscribe(res => { 
      if (res) { 
        const r = res as any; 
        this.selectedDate.set(r.date); // Fix pour le créneau
        
        this.form.patchValue({ 
          date: r.date, 
          selectedSlotId: r.selectedSlotId, // ID du créneau
          startTime: r.startTime, 
          endTime: r.endTime, 
          clientId: r.clientId, 
          clientName: r.clientName, 
          assignedServerIds: r.assignedServerIds || [], 
          assignedTeamIds: r.assignedTeamIds || [], // <--- CHARGEMENT DES ÉQUIPES
          status: r.status, 
  ...
  isTeamSelected(id: string): boolean { const current = this.form.value.assignedTeamIds as string[]; return current ? current.includes(id) : false; }
  toggleTeam(id: string) { const current = (this.form.value.assignedTeamIds as string[]) || []; let updated = current.includes(id) ? current.filter(tid => tid !== id) : [...current, id]; this.form.patchValue({ assignedTeamIds: updated }); }
  getSelectedTeamCount(): number { return (this.form.value.assignedTeamIds as string[])?.length || 0; }
  getTeamIcon(type: string) {
    switch(type) {
      case 'ORCHESTRE': return 'music_note';
      case 'PHOTOGRAPHE': return 'camera_alt';
      case 'TRAITEUR': return 'restaurant';
  ...
  async onSubmit() { 
    if (this.form.valid) { 
      ...
      } catch (e) { this.ui.showToast('error', 'Erreur sauvegarde'); } 
    } 
  }
  ...
  async deleteStaff(staff: any, event: Event) { event.stopPropagation(); const confirmed = await this.ui.confirm('Supprimer ?', 'Confirmer ?', 'Oui', 'Non'); if (confirmed) await this.staffService.delete(staff.id); }
  ...
  async saveQuickClient() { if (this.quickClientForm.valid) { const docRef = await this.clientService.add(this.quickClientForm.value as any); this.closeClientModal(); const newData = this.quickClientForm.value; this.selectClient({ id: docRef.id, ...newData }); } }
