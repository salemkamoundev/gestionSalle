...
// CORRECTION DES COMPOSANTS (../ au lieu de ../../)
...
export class ReservationFormComponent implements OnInit {
  ...
  selectClient(client: any) { 
    ...
    if (this.reservationId && this.selectedClientId() && this.selectedClientId() !== client.id) {
        if (!confirm('Êtes-vous sûr de changer le client ?')) {
            return;
        }
    }
  ...
  async loadPayments(reservationId: string) {
      try {
          this.paymentService.getByReservation(reservationId).subscribe((data: any[]) => {
