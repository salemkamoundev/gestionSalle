          ...
          <div class="p-6 space-y-4" formArrayName="creneaux">
            <div *ngFor="let slot of creneauxArray.controls; let i = index" [formGroupName]="i" 
              ...
              <div class="md:col-span-3">
                <label class="block text-xs font-bold text-slate-400 uppercase mb-1">Nom Période</label>
                <input formControlName="label" type="text" placeholder="Ex: Haute Saison" class="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm focus:border-blue-500 outline-none">
              </div>

              <div class="md:col-span-2">
                <label class="block text-xs font-bold text-slate-400 uppercase mb-1">Du</label>
                <input formControlName="validFrom" type="date" class="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm focus:border-blue-500 outline-none">
              </div>

              <div class="md:col-span-2">
                <label class="block text-xs font-bold text-slate-400 uppercase mb-1">Au</label>
                <input formControlName="validTo" type="date" class="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm focus:border-blue-500 outline-none">
              </div>

              <div class="md:col-span-2">
                <label class="block text-xs font-bold text-slate-400 uppercase mb-1">Horaire</label>
                <div class="flex items-center gap-1">
                   <input formControlName="start" type="time" class="w-full px-2 py-2 rounded-lg border border-slate-200 text-sm focus:border-blue-500 outline-none">
                   <span class="text-slate-400">-</span>
                   <input formControlName="end" type="time" class="w-full px-2 py-2 rounded-lg border border-slate-200 text-sm focus:border-blue-500 outline-none">
                </div>
              </div>
  ...
  async saveConfig() {
    if (this.configForm.valid) {
      try {
        ...
        this.ui.showToast('error', 'Veuillez corriger les erreurs');
