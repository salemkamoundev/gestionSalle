...
echo "Correction de la syntaxe dans $FILE_PATH..."

# Réécriture complète du fichier avec la syntaxe correcte
cat << 'EOF' > "$FILE_PATH"
  ...
  template: `
                ...
                @for (staff of servers(); track staff.id) { 
                  <div class="relative group rounded-lg border transition-all duration-200 select-none bg-white hover:shadow-md" 
                    ...
                    <div class="absolute top-1 right-1 flex opacity-0 group-hover:opacity-100 transition-opacity bg-white/80 rounded backdrop-blur-sm shadow-sm">
                      <button type="button" (click)="editStaff(staff, $event)" class="p-1 text-slate-400 hover:text-blue-600 rounded"><span class="material-icons text-[16px]">edit</span></button>
                      <button type="button" (click)="deleteStaff(staff, $event)" class="p-1 text-slate-400 hover:text-red-600 rounded"><span class="material-icons text-[16px]">delete</span></button>
                    </div>
                  </div> 
                }
              </div>
    ...
    @if (showClientModal()) { 
      <div class="fixed inset-0 z-[60] bg-black/50 flex items-center justify-center p-4">
        <app-client-form [isModal]="true" (finish)="onClientFormFinished($event)"></app-client-form>
      </div> 
    }
...
export class ReservationFormComponent implements OnInit {
    ...
    this.reservationService.getById(id).subscribe(res => { 
      if (res) { 
        ...
        this.form.patchValue({ 
          ...
          assignedServerIds: r.assignedServerIds || [], status: r.status, 
          totalPrice: r.totalPrice || 0, advance: r.advance || 0 
        }); 
  ...
  getSelectedServerCount(): number { 
    return (this.form.value.assignedServerIds as string[])?.length || 0; 
  }
  ...
  selectClient(client: any) { 
    this.searchTerm.set(`${client.nom} ${client.prenom}`); 
    this.form.patchValue({ clientId: client.id, clientName: `${client.nom} ${client.prenom}` }); 
  ...
  async deleteStaff(staff: any, event: Event) { 
    event.stopPropagation(); 
    const confirmed = await this.ui.confirm('Supprimer ?', `Supprimer ${staff.nom} ?`, 'Oui', 'Non'); 
    if (confirmed) { 
  ...
  // Methodes Client Modal
  openClientModal() { this.showClientModal.set(true); } 
  
  onClientFormFinished(newClient: any) {
  ...
  async saveQuickStaff() { 
    if (this.quickStaffForm.valid) { 
      const docRef = await this.staffService.add(this.quickStaffForm.value as any); 
      this.closeStaffModal(); 
      this.toggleServer(docRef.id); 
      this.ui.showToast('success', 'Staff ajouté'); 
    } 
  }
}
EOF

echo "Fichier ReservationFormComponent réparé !"