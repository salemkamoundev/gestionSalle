...
# Nom du script : add_cin_date_details.sh
# Description : Ajoute la Date de Délivrance (CIN) et peaufine l'affichage des détails client dans le formulaire de réservation.

HTML_FILE="src/app/features/calendar/reservation-form/reservation-form.component.html"

# 1. Sauvegarde
if [ ! -f "$HTML_FILE.bak_cin_date" ]; then
    cp "$HTML_FILE" "$HTML_FILE.bak_cin_date"
    echo "📦 Backup créé : $HTML_FILE.bak_cin_date"
fi

echo "🔧 Ajout de la date de délivrance CIN dans la fiche client..."
...
echo "✅ Fiche client complète avec Date de Délivrance ajoutée au formulaire !"