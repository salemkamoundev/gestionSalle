...
echo "📊 Mise à jour Dashboard : Séparation 'À venir' vs 'Impayés passés'..."

# 1. DASHBOARD TS : Création de deux signaux computed distincts
...
# 2. DASHBOARD HTML : Deux sections distinctes
        ...
        <tbody class="divide-y divide-slate-50">
          @for (res of upcomingReservations(); track res.id) {
            <tr class="hover:bg-slate-50 transition cursor-pointer" (click)="navigateToReservation(res.id)">
              ...
              <td class="px-6 py-4">
                <div class="text-xs text-slate-500 italic max-w-[150px] truncate" title="{{ res.notes }}">
                    {{ res.notes || '-' }}
                </div>
              </td>
...
echo "✅ Dashboard séparé : 'À venir' et 'Impayés' sont maintenant dans deux listes distinctes."