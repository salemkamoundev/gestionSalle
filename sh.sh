...
# ---------------------------------------------------------
# 1. MISE À JOUR DES DONNÉES FIRESTORE (Ajout Fin d'année)
# ---------------------------------------------------------
echo "📦 Mise à jour des données Firestore pour inclure Décembre 2025..."

if [ ! -f "serviceAccountKey.json" ]; then
    echo "⚠️ Attention: serviceAccountKey.json manquant. Assurez-vous d'être à la racine."
fi

cat > update_slots_full_year.js <<'EOF'
const admin = require('firebase-admin');
const serviceAccount = require('./serviceAccountKey.json');

// On couvre TOUTE l'année 2025
const PERIODS = [
  ...
  { id: 'automne_2025', label: 'Automne/Hiver 2025', from: '2025-10-01', to: '2025-12-31', coef: 1.0 }
];

const DAY_SLOTS = [
  { key: 'matin', label: 'Matin', start: '08:00', end: '12:00', basePrice: 600 },
  { key: 'aprem', label: 'Après-midi', start: '13:00', end: '17:00', basePrice: 800 },
  { key: 'soir',  label: 'Soir',  start: '18:00', end: '02:00', basePrice: 1200 },
];
...
async function updateConfig() {
  ...
  for (const p of PERIODS) {
    for (const s of DAY_SLOTS) {
      creneaux.push({
        id: `${p.id}_${s.key}`, // ex: automne_2025_matin
  ...
  console.log(`✅ ${creneaux.length} créneaux générés (Couverture Janvier -> Décembre 2025).`);
}

updateConfig().then(() => process.exit(0)).catch(e => { console.error(e); process.exit(1); });
EOF

# Exécution du seed si node est dispo
if command -v node &> /dev/null; then
    npm install firebase-admin --no-save >/dev/null 2>&1
    node update_slots_full_year.js
    rm update_slots_full_year.js
else
    echo "❌ Node.js non trouvé. Impossible de mettre à jour Firestore automatiquement."
fi

# ---------------------------------------------------------
# 2. MISE À JOUR DU COMPOSANT ANGULAR (Logique robuste)
# ---------------------------------------------------------
...
echo "🔧 Correction du code Angular..."

# Réécriture du TypeScript
cat > "$TS_FILE" << 'EOF'
...
# Réécriture du HTML (pour utiliser filteredSlots)
cat > "$HTML_FILE" << 'EOF'
                  ...
                  <div class="flex items-center gap-3 border-b pb-4 mb-4">
                    ...
                    <div>
                      ...
                      <div class="text-slate-500 text-sm flex items-center gap-1">
                        ...
                        @if (selectedClient()?.telephone2) {
                           <span class="text-slate-400 px-1">/</span>
                           <span class="text-slate-500">{{ selectedClient()?.telephone2 }}</span>
                        }
                      </div>
                    </div>
                  </div>
