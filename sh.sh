...
# 1. Mise à jour du Modèle (Array d'IDs)
...
# 2. Mise à jour du Formulaire de Réservation
TARGET_FORM="src/app/features/calendar/reservation-form/reservation-form.component.ts"
...
@Component({
  ...
  template: \`
          ...
          <div class="space-y-6">
            <div class="space-y-4">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select formControlName="selectedSlotId" (change)="onSlotChange(\$event)" 
                    ...
                    @if (availableSlots().length === 0 && selectedDate()) { <option value="" disabled>Complet ou indisponible</option> }
                  </select>
                </div>
              </div>
              ...
              <div class="relative z-20"> 
                ...
                <div class="flex gap-2">
                  ...
                  <button type="button" (click)="openClientModal()" class="bg-emerald-500 hover:bg-emerald-600 text-white px-3 rounded-lg shadow transition"><span class="material-icons">person_add</span></button>
                </div>
            ...
            <div class="space-y-4">
              <div class="flex justify-between items-center border-b border-slate-100 pb-2">
                ...
                @if(isEditMode()) {
                  <button type="button" (click)="openPaymentModal()" class="text-xs bg-purple-100 text-purple-700 px-3 py-1 rounded font-bold hover:bg-purple-200 transition flex items-center">
                    <span class="material-icons text-sm mr-1">payments</span> Gérer les Règlements
                  </button>
                }
              </div>
              <div class="grid grid-cols-2 gap-4">
                ...
                <div><label class="block text-sm font-bold text-slate-700 mb-1">Avance Reçue (TND)</label><input formControlName="advance" type="number" class="w-full px-4 py-2 border border-emerald-300 bg-emerald-50 rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none font-mono font-bold text-emerald-700 text-right text-lg" readonly></div>
              </div>
            </div>
          </div>
            ...
            <div class="space-y-4">
              <div class="flex items-end justify-between border-b border-slate-100 pb-2">
                ...
                <div class="flex bg-slate-100 p-1 rounded-lg">
                  <button type="button" (click)="assignMode.set('STAFF')" class="px-3 py-1 text-xs font-bold rounded-md transition-all duration-200" [class.bg-white]="assignMode() === 'STAFF'" [class.text-blue-600]="assignMode() === 'STAFF'" [class.shadow-sm]="assignMode() === 'STAFF'" [class.text-slate-500]="assignMode() !== 'STAFF'">Staff</button>
                  <button type="button" (click)="assignMode.set('TEAM')" class="px-3 py-1 text-xs font-bold rounded-md transition-all duration-200" [class.bg-white]="assignMode() === 'TEAM'" [class.text-purple-600]="assignMode() === 'TEAM'" [class.shadow-sm]="assignMode() === 'TEAM'" [class.text-slate-500]="assignMode() !== 'TEAM'">Équipe</button>
                </div>
              </div>

              @if (assignMode() === 'STAFF') {
                <div class="animate-fade-in">
                  <div class="flex justify-between items-center mb-2"><span class="text-xs text-slate-500 italic">Serveurs internes</span><span class="text-xs font-bold px-2 py-1 rounded bg-blue-50 text-blue-600">{{ getSelectedServerCount() }} sel.</span></div>
                  <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-80 overflow-y-auto pr-1">
                    @for (staff of servers(); track staff.id) { 
                      <div class="relative group rounded-lg border transition-all duration-200 select-none bg-white hover:shadow-md cursor-pointer" [class.border-blue-500]="isServerSelected(staff.id!)" [class.bg-blue-50]="isServerSelected(staff.id!)" (click)="toggleServer(staff.id!)">
                        <div class="p-2 flex items-center space-x-3"><div class="w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs transition-colors" [class.bg-blue-500]="isServerSelected(staff.id!)" [class.text-white]="isServerSelected(staff.id!)" [class.bg-slate-200]="!isServerSelected(staff.id!)" [class.text-slate-500]="!isServerSelected(staff.id!)">{{ isServerSelected(staff.id!) ? '✓' : staff.nom.charAt(0) }}</div><div class="flex-1 min-w-0"><p class="text-sm font-bold truncate">{{ staff.nom }}</p><p class="text-[10px] text-slate-500 truncate">{{ staff.specialite }}</p></div></div>
                      </div> 
                    }
                  </div>
                </div>
              ...
              @if (assignMode() === 'TEAM') {
                <div class="animate-fade-in">
                  <div class="flex justify-between items-center mb-2">
                    <span class="text-xs text-slate-500 italic">Prestataires externes</span>
                    <span class="text-xs font-bold px-2 py-1 rounded bg-purple-50 text-purple-600">{{ getSelectedTeamCount() }} sel.</span>
                  </div>
                      ...
                      <div class="relative group rounded-lg border transition-all duration-200 select-none bg-white hover:shadow-md cursor-pointer"
                        ...
                        <div class="p-3 flex items-center space-x-3">
                          <div class="w-10 h-10 rounded-lg flex items-center justify-center font-bold text-lg transition-colors"
                               ...
                               @if(isTeamSelected(team.id!)) { <span class="material-icons text-sm">check</span> } 
                               @else { <span class="material-icons text-sm">groups</span> }
                          </div>
    ...
    @if (showPaymentModal()) { <app-payment-modal [reservation]="currentReservation()" (onClose)="closePaymentModal()"></app-payment-modal> }
  \`,
...
export class ReservationFormComponent implements OnInit {
  ...
  showPaymentModal = signal(false); currentReservation = signal<Reservation | null>(null);
  ...
  availableSlots = computed(() => { 
    ...
    const otherReservations = this.isEditMode() && this.reservationId ? occupied.filter(r => String(r.id) !== String(this.reservationId)) : occupied;
  ...
  // FORMULAIRE MIS À JOUR : assignedTeamIds est un tableau
  form = this.fb.group({ 
    ...
    assignedTeamIds: [[] as string[]], // <--- MODIFICATION ICI (Tableau)
    status: ['CONFIRMED'], 
    totalPrice: [0], 
    advance: [0] 
  });
  ...
  fetchReservationsForDate(dateStr: string) { this.reservationService.getByDate(dateStr).subscribe(res => { this.existingReservations.set(res); }); }
  trySelectSlot(time: string) { const slots = this.availableSlots(); let match = slots.find(s => s.start === time); if (!match && slots.length > 0) { const hour = parseInt(time.split(':')[0], 10); if (hour < 12) match = slots.find(s => parseInt(s.start.split(':')[0]) < 12); else if (hour < 18) match = slots.find(s => parseInt(s.start.split(':')[0]) >= 12 && parseInt(s.start.split(':')[0]) < 18); else match = slots.find(s => parseInt(s.start.split(':')[0]) >= 18); } if (match) { this.form.patchValue({ selectedSlotId: match.id, startTime: match.start, endTime: match.end, totalPrice: match.price }); this.isPriceAutoUpdated.set(true); } }
  onDateChange() { const newDate = this.form.value.date || ''; this.selectedDate.set(newDate); this.form.patchValue({ selectedSlotId: '', startTime: '', endTime: '', totalPrice: 0 }); if (newDate) this.fetchReservationsForDate(newDate); }
  ...
  loadReservation(id: string) { 
    this.reservationService.getById(id).subscribe(res => { 
      if (res) { 
        ...
        // MIGRATION / COMPATIBILITÉ : Si ancienne structure avec ID unique
        let teamIds = r.assignedTeamIds || [];
        if (!teamIds.length && r.assignedTeamId) {
          teamIds = [r.assignedTeamId];
        }
        ...
        this.form.patchValue({ 
          ...
          assignedTeamIds: teamIds, // Charge le tableau
          status: r.status, 
        ...
        this.fetchReservationsForDate(r.date); 
        this.searchTerm.set(r.clientName || '');
        
        // Switcher si des équipes sont présentes
        if (teamIds.length > 0) this.assignMode.set('TEAM');
        
        if (!r.selectedSlotId && r.startTime) setTimeout(() => this.trySelectSlot(r.startTime), 500);
      } 
    }); 
  }
  ...
  // --- NOUVELLE LOGIQUE MULTI-ÉQUIPES ---
  isTeamSelected(id: string): boolean { 
    const current = this.form.value.assignedTeamIds as string[]; 
    return current ? current.includes(id) : false; 
  }
  
  toggleTeam(id: string) { 
    const current = (this.form.value.assignedTeamIds as string[]) || []; 
    // Ajout ou Suppression (Toggle)
    let updated = current.includes(id) ? current.filter(tid => tid !== id) : [...current, id]; 
    this.form.patchValue({ assignedTeamIds: updated }); 
  }
  
  getSelectedTeamCount(): number { 
    return (this.form.value.assignedTeamIds as string[])?.length || 0; 
  }
  
  clearTeam() { this.form.patchValue({ assignedTeamIds: [] }); }
  // -------------------------------------
  ...
  openPaymentModal() { if (!this.isEditMode() || !this.reservationId) { this.ui.showToast('info', 'Enregistrez d\\'abord'); return; } const currentValues = this.form.value; this.currentReservation.update(prev => ({ ...prev!, ...currentValues, id: this.reservationId! } as any)); this.showPaymentModal.set(true); }
  closePaymentModal() { this.showPaymentModal.set(false); if (this.reservationId) this.loadReservation(this.reservationId); }
}
EOF

echo "Mise à jour : Sélection multiple des équipes activée."