      ...
      <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <div class="overflow-x-auto">
          <table class="w-full text-left">
            ...
            <tbody class="divide-y divide-slate-100">
              @for (staff of paginated(); track staff) {
        ...
        <div class="data-pagination-footer flex flex-col md:flex-row gap-3 items-center justify-between px-6 py-3 border-t border-slate-200 bg-slate-50">
          <div class="text-sm text-slate-500">
            ...
            <span class="font-semibold">{{ this.filteredStaff().length }}</span> résultats
          </div>
...
import { Router, RouterModule } from '@angular/router';
  ...
  // --- Admin staff filters + redirect to chat ---
  searchTerm = '';
  ...
  filteredStaff(list: any[]): any[] {
    const term = (this.searchTerm || '').toLowerCase().trim();
    if (!term) return list || [];
    return (list || []).filter((s: any) => {
      const name = String(s?.name || s?.fullName || s?.prenom || s?.nom || '').toLowerCase();
      const email = String(s?.email || '').toLowerCase();
      const phone = String(s?.phone || s?.tel || s?.telephone || '').toLowerCase();
